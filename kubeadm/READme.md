...
``` sh
# Initialize Kubernates master by executing below command.
# Configure kubectl command line tool that let you control the cluster
# For configuration, kubectl looks for a file named config in the $HOME/.kube

sudo kubeadm init
mkdir -p $HOME/.kube                                              #Create the directory 
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config          #Copy the config file generated by the kubedam to $HOME/.kube/
sudo chown $(id -u):$(id -g) $HOME/.kube/config                   #Change ownership of the file to the current logged in user
```
...
```
```sh
#deploy the network plugin - weave network (or other open source network add-on like Calico) 
kubectl apply -f https://github.com/weaveworks/weave/releases/download/v2.8.1/weave-daemonset-k8s.yaml
kubectl get pods -A
kubectl get node
```
```sh
## To join other nodes as a worker nodes to the cluster, we need the token generated by the kubeadm init command
## Run the following command on the Control Plane to get the token
...
--discovery-token-ca-cert-hash sha256:1fc51686afd16c46102c018acb71ef9537c1226e331840e7d401630b96298e7d         #Output of the token created
```
